<?php
    ...
    $totalPages = 1;
    if ($userRole == "user") {
        // Query database hanya untuk data pengguna saat ini
        $NIM = $_SESSION["NIM"];
        $sql = "SELECT * FROM AkarKuadrat WHERE username = '$NIM'";
        ...
        if ($result !== null && $result->num_rows > 0) {
            ...
            $sql = "SELECT * FROM AkarKuadrat WHERE username = '$NIM' LIMIT $dataPerPage OFFSET $offset";
            $result = $conn->query($sql);
        } else {
                ...
                <?php
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            <td>".$row["Metode"]."</td>
                            <td>".$row["Waktu"]."</td>
                            <td><a class='delete-icon' href='alert_delete_per_pengguna.php?ID=" . $row["ID"] . "'>🗑️</a></td>
                            </tr>";
                        }
                        echo "</table>";
                    } else {
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination">
                <?php
                    ...
                    if ($currentPage > 1) {
                        echo "<li class='page-item'><a class='page-link' href='view_per_pengguna.php?page=1'>&laquo;</a></li>"; // Tanda panah untuk ke halaman pertama
                        echo "<li class='page-item'><a class='page-link' href='view_per_pengguna.php?page=" . ($currentPage - 1) . "'>&laquo;</a></li>"; // Tanda panah untuk halaman sebelumnya
                    }

                    for ($i = $startPage; $i <= $endPage; $i++) {
                        echo "<li class='page-item " . ($i == $currentPage ? 'active' : '') . "'><a class='page-link' href='view_per_pengguna.php?page=$i'>$i</a></li>";
                    }

                    if ($currentPage < $totalPages) {
                        echo "<li class='page-item'><a class='page-link' href='view_per_pengguna.php?page=$totalPages'>&raquo;</a></li>"; // Tanda panah untuk ke halaman terakhir
                        echo "<li class='page-item'><a class='page-link' href='view_per_pengguna.php?page=$totalPages'>&raquo;&raquo;</a></li>"; // Tanda panah untuk ke halaman terakhir
                    }
                ?>
            </ul>
        </nav>
